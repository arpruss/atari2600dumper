...
#define SAFER_MODE // avoid even a short instance of having a pullup resistor on a potential 5V line
                   // This could in rare circumstances produce a false-negative for detectFA(). 
                   // It's probably safe to comment this out.
bool hotplug = true;
bool stellaExt = true;
bool finicky = true;
...
void switchBankFA(uint32_t bank) {
  ...
  pinMode(dataPins[0], INPUT_PULLUP); // D0 must be high to switch banks on FA: https://patents.google.com/patent/US4485457A/en
              ...
              // If pin 0 is asserted by the cartridge (which may happen on 4K and 2K cartridges) this violates the STM strictures 
              // against pulling up potential 5V data, but we only do it for 4 microseconds. Ideally, this code won't be called
              // except for an FA cart, which should float the pin.
...
// checks if there is a write port in address:address+size
// returns 1 if yes, 0 if no, and -1 if maybe 
int detectWritePort(unsigned _address, unsigned size=0x80) {
  for (unsigned address = address; address < _address + size; address++) {
    *addressBit12Write = 0; 
...
uint16_t detectFA() {
  if (detectWritePort(0x1000,0x100) 
#ifdef SAFER_MODE  
      <= 
#else
      ==
  ...
  // at this point we are pretty sure it has to be FA, but just in case it's something
  // that fell through the cracks, let's do another check. A danger in doing that check
  // is that switching banks may result in activating pull-up on a 5V signal for a few
  // microseconds. That should be OK, and I can't right now think of a different way to do 
  // it.
  if (! diff(1,2))
    return 0;
  return 0xFA;
